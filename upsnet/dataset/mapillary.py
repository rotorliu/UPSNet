...
class Mapillary(BaseDataset):
    ...
    def __init__(self, image_sets, flip=False, proposal_files=None, phase='train', result_path=''):
        ...
        image_dirs = {
            'train': os.path.join(config.dataset.dataset_path, 'training', 'images'),
            'val': os.path.join(config.dataset.dataset_path, 'validation', 'images'),
            'test': os.path.join(config.dataset.dataset_path, 'testing', 'images'),
        }

        anno_files = {
            'train': os.path.join(config.dataset.dataset_path, 'training', 'panoptic', 'panoptic_2018.json'),
            'val': os.path.join(config.dataset.dataset_path, 'validation', 'panoptic', 'panoptic_2018.json'),
        }
    ...
    def evaluate_ssegs(self, pred_segmentations, res_file_folder):
        ...
        for i, roidb in enumerate(self.roidb):
            ...
            seg_gt = np.array(Image.open(self.roidb[i]['image'].replace('images', 'panoptic').replace('jpg', 'png'))).astype(np.float32)
